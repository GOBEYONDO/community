...
public interface CommentDao {
    @Insert("insert into comment(parent_id,type,comment_id,gmt_create,gmt_modified,like_count,content,question_id) values(#{parent_id},#{type},#{comment_id},#{gmt_create},#{gmt_modified},#{like_count},#{content},#{question_id})")
    ...
    public List<Comment> findByParentIdAndType(Map<String,Object> map);

    @Select("select * from comment where question_id = #{question_id} order by gmt_create desc")
    @Results({
            @Result(id = true ,column = "id",property = "id"),
    ...
    public List<Comment> findAll(Integer question_id);
